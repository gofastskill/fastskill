...
# Step 4: Install the Helm chart (unpack the chart archive from GitHub Releases or clone the repo)
export CHART_PATH=./fastskill-chart   # path where you extracted the chart
helm install fastskill "$CHART_PATH" \
  --namespace fastskill \
  --set s3.bucket=my-fastskill-registry \
  --set s3.region=us-east-1 \
  --set s3.blobBaseUrl=https://my-fastskill-registry.s3.amazonaws.com/skills

# Or use your own values file next to the chart
helm install fastskill "$CHART_PATH" \
  -f ./values-registry-s3.yaml \
  --namespace fastskill
...
helm upgrade fastskill "$CHART_PATH" \
  -f ./values-prod.yaml \
  --namespace fastskill
...
1. **Registry Deployment**: Runs `fastskill serve` (HTTP API and bundled UI)
...
The Helm chart ships in the [FastSkill GitHub repository](https://github.com/gofastskill/fastskill) under `tools/fastskill/helm/fastskill`. Download a release bundle or sync that folder locally, set `CHART_PATH` to the directory you unpacked, then run the `helm` commands above.
