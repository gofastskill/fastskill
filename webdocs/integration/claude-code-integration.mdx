...
After running the command, reload Claude Code. The tools exposed by `fastskill mcp serve --transport stdio` will be callable from the agent — the read-only ones only. Mutating tools (`fastskill_install`, `fastskill_add`, `fastskill_remove`, …) require `mcp serve --enable-write`; see [Write access](/tool-calling/development#write-access).
