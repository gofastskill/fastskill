...
This workflow is the day-to-day path for quality checks. If you haven't authored a suite yet — the `[tool.fastskill.eval]` table, prompts CSV, and checks TOML — start with [Eval setup](/evals-quality/setup) first; pass/fail behavior (especially the `should_trigger` column and the `claude`-agent `trigger_expectation` caveat) is documented there.
