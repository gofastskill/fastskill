...
Matches a **structured `Skill` tool invocation** in the trace, not a substring of prose. When `skill` is given, that name must appear in the invocation's serialized input. This is the robust way to assert triggering — and, with `expected = false`, the only reliable way to assert non-triggering.
