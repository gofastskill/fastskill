...
The checks file is loaded by `aikit_evals::checks::ChecksToml`: an array of `[[check]]` tables, each identified by a `name` field. There are exactly four check types:

| `name` | Fields | Description |
|---|---|---|
| `trigger_expectation` | `pattern` (string), `expected` (bool), `required` (bool, default `true`) | Passes if `pattern` appearing in the captured output matches `expected` (found == expected). |
| `command_contains` | `pattern` (string), `required` (bool, default `true`) | Passes if `pattern` appears anywhere in the captured stdout/trace. |
...
- `hard` — 1.0 if every check passed for that case, else 0.0.
- `soft` — the fraction of checks that passed for that case.
- `mixed` — a weighted blend of the hard and soft scores.

There is no per-check weighting — every check counts equally within a case.

### `should_trigger` is not the grading signal

`should_trigger` is a **required CSV column**, but it is not read by any scorer. Pass/fail is determined entirely by `checks.toml`. If you want the optimizer to actually check that a case did or didn't trigger the skill, encode that as a `trigger_expectation` check with `expected` set to match — the CSV column alone does nothing.
