...
| `max_command_count` | `limit` (int), `required` (bool, default `true`) | Passes if the number of tool invocations the agent made does not exceed `limit`. (Internally this is the `max_tool_calls` check; `max_command_count` is accepted as an alias.) |
