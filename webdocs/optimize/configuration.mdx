...
A case with zero configured checks scores `1.0` under all three metrics (vacuously) — this is
the same "no signal without `checks`" gap described in [Setup](/optimize/setup).
