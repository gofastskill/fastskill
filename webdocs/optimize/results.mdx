...
directory. `--show diffs` for step N diffs `skill_v{N}.md` against `skill_v{N+1}.md`.

**A new version is only written when a step is accepted.** So for a rejected step there is
no "after" document to diff against, and `inspect` says so rather than inventing one:

```
(no diff available: skill_v0001.md does not exist — step 0 likely was not accepted, so no
new version was written; run `optimize inspect --step 0 --show gate` to check)
```

This also means version numbers stop tracking step numbers one-for-one once any step in the
run has been rejected. The mapping is exact for step 0, and for any run where every step up
to N was accepted.
...
| `skills/skill_{NNNN}.md` | Versioned skill documents; a new one is written per accepted step |
