...
| **Memory** | 512MB | 2GB+ |
| **Storage** | 100MB | 500MB+ |
| **OpenAI API Key** | Optional | Required for embedding search and `index rebuild` |
...
Options (run `./install.sh --help` after downloading the script): `--user`, `--prefix DIR`, `--force`, and a version argument such as `./install.sh v0.9.208`.

</Tab>

<Tab value="GitHub Releases">

Download a pre-built binary from [GitHub Releases](https://github.com/gofastskill/fastskill/releases). Replace `VERSION` with the tag you want (for example `0.9.205`; assets use `v${VERSION}` in the URL).
...
- Install with `fastskill project install` (use `--without`/`--only` for groups)
...
If you already have a project with `skill-project.toml`, try `fastskill skill list` or `fastskill project install` from that directory.
