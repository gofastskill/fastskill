...
monitoring = { origin = { type = "repository", repo = "team-tools", skill = "monitoring", version = "1.0.0" } }
...
Every dependency names an **origin** — where the skill comes from. Four kinds:

**Git Repository:**
```toml
[dependencies]
git-skill = { origin = { type = "git", url = "https://github.com/org/skill.git", ref = { branch = "main" } } }
```

**Local Path:**
```toml
[dependencies]
local-skill = { origin = { type = "local", path = "./local-skill" } }
```

**ZIP URL:**
```toml
[dependencies]
zip-skill = { origin = { type = "zip-url", url = "https://example.com/skill.zip" } }
```

**Repository:**
```toml
[dependencies]
repo-skill = { origin = { type = "repository", repo = "public-registry", skill = "skill-name", version = "1.0.0" } }
```

**With Groups:**
```toml
[dependencies]
dev-skill = { origin = { type = "git", url = "https://github.com/org/dev-skill.git" }, groups = ["dev"] }
```

**Editable Install:**
```toml
[dependencies]
editable-skill = { origin = { type = "local", path = "./skill", editable = true } }
```
...
web-scraper = { origin = { type = "git", url = "https://github.com/org/repo.git" } }
...
**Install commands:**
- `fastskill project install` - Installs all skills (default + all groups)
- `fastskill project install --without dev` - Installs all except dev group
- `fastskill project install --only prod` - Installs only prod group
...
```toml
[dependencies]
my-local-skill = { origin = { type = "local", path = "./local-skills/my-skill", editable = true }, groups = ["dev"] }
```
...
The Lock carries the same `origin` the Manifest declares (install *intent*) plus a
`resolved` table holding what the fetch actually produced — the concrete version, and the
exact `commit_hash` or `checksum` when there is one.
...
- Run `fastskill skill add <skill>`
- Run `fastskill skill update`
- Run `fastskill skill remove <skill>`
...
fastskill skill add --group dev pytest
...
```toml
[dependencies]
versioned-skill = { origin = { type = "zip-url", url = "https://example.com/skill.zip" } }
```
...
[dependencies]
web-scraper = { origin = { type = "git", url = "...", ref = { branch = "main" } } }
...
[dependencies]
other-skill = "1.0.0"
another-skill = { origin = { type = "repository", repo = "team-tools", skill = "another", version = "2.0.0" } }
```
...
- Created using `fastskill project init` command in the skill directory
...
```bash
fastskill skill add ./my-skill -e --group dev
```
