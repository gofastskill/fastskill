...
| `missing` | A selected root or required dependency is absent | Run `fastskill install` |
| `extraneous` | Installed content has no managed owner | Review it; this alone does not fail `--check` |
| `mismatch` | Desired, locked, or actual version/content differs | Restore with `install --lock` or deliberately `update` |
...
fastskill list --check --without dev --json > skills-status.json
```
...
### 1. Commit both manifest and lockfile
...
<Accordion id="extraneous-skills-accumulate" title="Extraneous skills accumulate">
  <Callout type="info">
...
<Accordion id="persistent-version-mismatches" title="Persistent version mismatches">
  <Callout type="warn">
...
- [init Command](/configuration/init-command) - Project initialization and setup
