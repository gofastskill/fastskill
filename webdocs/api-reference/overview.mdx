...
If you embed the Rust library, refer to the `fastskill` crate on [crates.io](https://crates.io/crates/fastskill) and the upstream repository for API details.
...
Local `fastskill serve` endpoints are unauthenticated. Requests to `/api/*` and `/v1/*` do not require JWT or API-key headers.
...
#### Registry endpoints (`fastskill serve`)
...
# Publish skill package (no auth headers required on local serve)
...
#### Manifest endpoints (`fastskill serve`)
...
Then open `http://localhost:8080/` in your browser (see [serve Command](/cli-reference/serve-command) for routes).
...
<Accordion title="Authentication Issues">
  <Info>
  Local `fastskill serve` does not enforce built-in authentication. If access control is required, place the service behind a reverse proxy or private network boundary.
  </Info>
</Accordion>
</AccordionGroup>

<Note>
The HTTP API matches what the local `fastskill serve` process exposes. Use it from any language or tool that can send HTTP requests.
</Note>
