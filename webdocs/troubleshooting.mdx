...
3. **Set XDG_CONFIG_HOME explicitly** (advanced):
   ```bash
   # Linux/macOS
...
**Symptom**: Every time you run `fastskill project install` or `fastskill skill add`, `skills.lock` shows changes in git even though no dependencies changed.
