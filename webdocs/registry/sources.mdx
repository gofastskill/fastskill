...
fastskill sources add my-skills-repo \
...
```bash
# Add repository
fastskill sources add company-tools \
  --repo-type zip-url \
...
fastskill sources add local-dev \
  --repo-type local \
  --url ~/local-skills \
  --priority 0
...
fastskill sources add external-dev \
  --repo-type git-marketplace \
...
- name: Test local skills
  run: |
    fastskill sources add test-repo \
      --repo-type local \
      --url ./test-skills
...
```bash
fastskill sources add new-source --type git-marketplace --url https://github.com/user/repo.git --priority 0
```
