...
fastskill marketplace create --path ./skills --name my-skills-repo
...
Use `fastskill repos skills` to list skills from HTTP registries. The command calls `GET /api/registry/index/skills` on the configured registry's `index_url`.
  ...
  package-and-deploy:
    ...
    steps:
      ...
      - name: Generate marketplace.json
        run: fastskill marketplace create --path ./skills --output ./marketplace.json --name company-tools
...
fastskill marketplace create --path ~/local-skills --name local-dev
```
...
FastSkill validates skills automatically when you add or install them, so there is no separate validate step:

```bash
# Adding from a local source surfaces validation errors before agents load the skill
fastskill add ~/local-skills

# Installing the manifest validates every resolved skill
fastskill install
```
...
- name: Test local skills
  run: |
    ...
    # fastskill add validates the skill before installing it
...
```bash
fastskill repos add new-source --repo-type git-marketplace --priority 0 https://github.com/user/repo.git
```
