...
fastskill marketplace create --path ./skills --output marketplace.json --name team-tools
```

Listing and inspecting skills from configured repositories:

```bash
# List skills from repositories
fastskill repos skills

# Show skill from repository
fastskill repos show pptx

# Show available versions of a skill
...
fastskill repos add private-registry \
  ...
  https://private.example.com/index \
  --auth-type api_key \
  --auth-env REGISTRY_API_KEY
```
