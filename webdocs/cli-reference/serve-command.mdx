...
| `/api/v1/skills/update` | POST | **write** | Update one (`{ "skillId": "..." }`) or all skills recorded in the project from their recorded origin as one compatible dependency plan; `{ "check": true }` reports `would_update`, `up_to_date`, or `immutable` without applying it. `/api/v1/skills/upgrade` is kept mounted as a back-compat alias for this same endpoint. |
