...
3. Stop the background `ValidationWorker` (if registry is configured).
...
| `/api/v1/registry/publish` | POST | Publish package (anonymous scope) |
...
| `/v1/skills` | GET/POST/DELETE | Claude-API compatible skills surface (unchanged) |
