...
**Breaking change:** `fastskill server serve` is **read-only by default**. Read endpoints (list/get skills,
...
> install/update/remove flow (and the CLI `fastskill skill add`/`skill update`/`skill remove`).
