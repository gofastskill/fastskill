...
# Test search
fastskill search "test" --local --embedding false --skills-dir .claude/skills
```
...
After installs in automation, run `fastskill reindex` with the same environment variables you use locally (`OPENAI_API_KEY`, optional `--skills-dir`). See [CI/CD integration](/integration/cicd-pipelines) for runner patterns.
...
```bash
fastskill search "process documents" --local --skills-dir .claude/skills/
```
