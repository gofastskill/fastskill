...
<CardGroup cols={2}>
  <Card title="fastskill search" icon="search">
    Semantic search across installed/available skills (supports `--format xml`).
  </Card>
  <Card title="fastskill sources" icon="book">
    Manage repositories (list/add/remove/show/update/test/refresh/create) with authentication and priority.
  </Card>
  <Card title="fastskill auth" icon="key">
    Manage authentication (login/logout/whoami) with JWT tokens and RBAC.
  </Card>
  <Card title="fastskill registry" icon="book">
    Legacy alias for sources command (add/list/remove/show/update/test/refresh/list-skills/show-skill/versions/search).
  </Card>
</CardGroup>
