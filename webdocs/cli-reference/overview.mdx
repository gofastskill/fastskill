...
<CardGroup cols={2}>
  ...
  <Card title="fastskill serve" icon="server">
    Start the HTTP server (use `--enable-registry` for the web UI). See [serve Command](/cli-reference/serve-command).
  </Card>
</CardGroup>
...
<CardGroup cols={2}>
  <Card title="fastskill add" icon="plus">
    Add skills from registry ID, git, zip, or local folder (supports `--branch`, `--tag`, `-e`, `--group`). See [skill commands](/cli-reference/skill-commands#add).
  </Card>
  ...
  <Card title="fastskill list" icon="list">
    List locally installed skills with reconciliation status (supports `--json`, `--grid`). See [skill commands](/cli-reference/skill-commands#list).
  </Card>
