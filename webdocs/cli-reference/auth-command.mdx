...
- Publishing skills to registries in `registry-publish` builds
...
# 2. Publish a skill
fastskill publish --artifacts ./artifacts/pptx-1.2.3.zip --api-url https://registry.example.com
...
fastskill publish --artifacts ./artifacts/pptx-1.2.3.zip --api-url https://registry.example.com
# Remote registry rejects the request if it requires authentication
```
...
fastskill publish --api-url https://prod-registry.example.com --artifacts ./artifacts/pptx-1.2.3.zip

# Publish to staging
fastskill publish --api-url https://staging-registry.example.com --artifacts ./artifacts/pptx-1.2.3.zip
```
...
# 4. Publish (requires a registry-publish build; token is used automatically)
...
fastskill publish --api-url https://staging.example.com --artifacts ./artifacts/pptx-1.2.3.zip

# 4. Promote to production
fastskill publish --api-url https://prod.example.com --artifacts ./artifacts/pptx-1.2.3.zip
```
...
<Accordion title="Multiple registries confused">
  <Tip>
  **Specify registry explicitly**: Use `--api-url` for registry API URLs or `--registry` for configured repository names.
  ```bash
  fastskill publish --api-url https://prod.example.com --artifacts ./artifacts/pptx.zip
  ```
  </Tip>
...
- [Publish Command](/cli-reference/publish-command) - Optional registry publishing
- [repos Command](/cli-reference/repository-command) - Managing repositories
