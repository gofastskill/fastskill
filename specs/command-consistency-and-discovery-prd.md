...
The command audit at `2b649bf` found that advertised update controls are ignored,
global updates only change timestamps, update failures return exit zero, and list
does not detect version drift. Search can return results that add cannot select;
...
1. As a user, I install the exact repository result I selected, without guessing
   which default repository add will use.
...
- **C-01:** Retain the current command families. Add records direct intent and
  installs its closure; install restores selections; update deliberately resolves
  changes; remove removes ownership; list/read reconcile and explain state. A
  repeated identical add MAY return unchanged, but MUST NOT upgrade a bundle or
  silently adopt a conflicting origin. Identity and ownership use the shared core.
- **C-02:** Add `--repository NAME` to add and update for repository-backed targets.
  Search results MUST contain canonical ID, repository identity, available version,
...
- **C-03:** The existing ambiguous `update --source` MUST become a documented
  deprecated alias of `--repository`; different values for both MUST fail. Changing
  repository through update requires one explicitly named repository-backed skill
  ...
  changes continue through explicit add replacement with ownership checks.
- **C-04:** `update ID --to-version VERSION` MUST require one named repository-backed
...
- **C-05:** Without `--to-version`, update MUST preserve recorded constraints.
...
- **C-06:** Global update MUST fetch, validate and apply actual content changes
...
- **C-07:** Add/install/update/remove and override operations MUST offer consistent
  `--dry-run` and `--json` results. Update `--check` renders a concise version of
...
- **C-10:** Consistent indexing policy MUST apply to individual and bundle paths.
  ...
  Explicit offline mode follows R-06. Previews MUST NOT reindex.

### Reconciliation and discovery

- **C-11:** List MUST compare desired constraints, locked facts, and actual identity,
...
- **C-12:** Add `list --check`; return nonzero for missing required contents,
...
- **C-13:** Read MUST use the selected installation scope and resolved Lock root.
  Installed read accepts an exact canonical ID; unsupported `ID@version` selection
...
- **C-14:** Repos skills/show/versions/search MUST use each adapter's supported
  ...
  Repos test MUST distinguish connectivity/catalog validity from acquisition
  capability, rather than imply unsupported downloads will work.
- **C-15:** Search MUST distinguish no repositories, zero matches, partial results,
...
| --- | --- |
| init | Create project/skill metadata; explicit overwrite behavior for existing files. |
| repos add/remove/update/list/info/test/refresh | Manage and inspect repository configuration and metadata. Removing configuration does not uninstall skills; references to removed configuration remain visible as unresolved provenance. |
| cache info/clean | Inspect/remove reusable acquisition cache; preserve managed ownership and required retained artifacts. |
| reindex | Maintain derived embeddings under the optional-provider contract. |
| doctor | Report selected-scope and effective provider/tool readiness; do not warn about an unrelated provider's missing token. Dependency drift belongs to list. |
| analyze matrix/cluster/duplicates | Inspect the selected index through distinct analysis views. |
...
| optimize run/resume/status/inspect/export | Run and inspect optimization; export remains an explicit artifact write. |
| marketplace create | Generate the distinct plugin catalog format. |
| serve; mcp serve/install/list | Keep HTTP/browser and MCP/client-configuration roles distinct. |
| completion/spec | Generate discoverability and machine schemas from accurate command definitions. |

These families need the applicable validation, output, scope and effect contracts;
this PRD does not introduce new evaluation algorithms or rename the command tree.

## Acceptance scenarios

| ID | Scenario | Required observable result |
| --- | --- | --- |
| C-A01 | Search a nondefault repository and run the returned add reference | Same canonical skill and repository selected; provenance retained. |
| C-A02 | Global local-origin v1 changes to v2; update | Disk changes to verified v2; timestamp-only success is impossible. |
...
| C-A07 | Change installed v2 to v9 or edit bytes without changing version | List identifies drift; check returns nonzero with desired/locked/actual evidence. |
...
| C-A09 | Group-limited install and check with matching selectors | Selected closure passes; unselected groups explicitly excluded. |
| C-A10 | Locked metadata read from nested/global scope | Same selected state as its root invocation; no ambient project leakage. |
| C-A11 | Search no query, empty matches, failed repository, partial results | No panic; distinct statuses, nonzero incomplete/error results, valid JSON. |
...
generated help/spec parity tests, but do not treat their success as lifecycle proof.
