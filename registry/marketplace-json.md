...
**Note**: FastSkill will automatically try the Claude Code standard location (`.claude-plugin/marketplace.json`) first, then fall back to the root location if not found. Both locations are supported.
...
* `--repo-version <version>`: Repository version (optional)
