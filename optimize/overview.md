...
`fastskill optimization` is an automated loop that improves a skill document by running evals, scoring results, and applying gradient-style patches until the skill reliably triggers on the cases you care about.
...
| -------------------------------- | -------------------------------------------------- |
| `fastskill optimization run`     | Start a new optimization run from a config file    |
| `fastskill optimization status`  | Check progress and scores for a run                |
| `fastskill optimization resume`  | Continue an interrupted run                        |
| `fastskill optimization inspect` | Examine a specific step's patch and gate decision  |
| `fastskill optimization export`  | Copy the best skill document to a destination path |
