...
Repository-ID examples below assume a configured catalog:
...
`--paths` always emits JSON. Editable installs resolve through their managed symlink to the source
directory, so their path fields are populated like copied installs.
